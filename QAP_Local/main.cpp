...
void runALSExperiment(vector<double> &currentResults,
	...
	while (t.elapsed_time(Timer::VIRTUAL) <= MAX_SECONS_PER_RUN
		...
		currentResults.push_back(currentFitness);
		bestSoFarResults.push_back(	min(bestSoFarResults.back(), currentFitness));
		//DUDA estara la funcion min hecha? max(bestSoFarResults.back(), currentFitness));


		//TODO Invoca a QAPLocalSearch::optimise para optimizar la solución
