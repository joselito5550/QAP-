...
protected:
	 ...
	 * _numInstalaciones Entero que indica el número de mochilas que se va a considerar. Este no se lee del fichero, sino que lo establecéis vosotros
	 * _numLocalizaciones Entero donde se almacenará el número de objetos del problema
...
public:
	...
	double getDistancia(int a, int b){
		return _distancias[a][b];
	};

	double getFlujo(int i, int j){
		return _flujos[i][j];
	};

	
	/**
	 * Función que devuelve la suma total de los costes
	 * @param[in] solution Referencia a un objeto que representa una solución al problema
	 * @return Suma de los costes
	 */
	double getSumCoste(QAPSolution &solution);
	 ...
	 *
	 */
	double getDeltaSumCoste(QAPSolution &solution, int indexIns, int indexLoc);
