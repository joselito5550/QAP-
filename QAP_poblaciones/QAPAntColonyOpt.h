		...
		void createAlternatives(
			...
			for (auto indexObj : _objectsLeft) {
				...
				for (unsigned j = 1;
					...
					aux=pow(density,beta)*pow((* phMatrix[indexObj])[j],alpha);
					alternatives.push_back(al);
					significances.push_back(aux);				}
			}
		}
		...
		void selectBestAlternative(MQKPObjectAssignmentOperation &op) {
			...
			for (auto indexObj : _objectsLeft) {
				...
				for (unsigned j = 1;
					...
					double aux=pow(density,beta)*pow((* phMatrix[indexObj])[j],alpha);

					if(aux > bestSignificance){
						op.setValues(indexObj,j,deltaFitness);
						bestSignificance = aux;
					}
				}
			}
		}
...
protected:
	...
	void iterate() {
		...
		for (unsigned i = 0; i < numObjs; i++) {
			_phMatrix[i]->at(_bestSolution->whereIsObject(i))=(1-_evaporation)*_phMatrix[i]->at(_bestSolution->whereIsObject(i))+_evaporation*fitness; //PRIMERA VERSION
		}
	}
